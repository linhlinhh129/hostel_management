<%@ page contentType="text/html;charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        ...
        <c:set var="displayName"
            value="${not empty sessionScope.currentUser.fullName ? sessionScope.currentUser.fullName : 'Người dùng'}" />
        <c:set var="roleLabel"
            value="${not empty sessionScope.currentUser.roleLabel ? sessionScope.currentUser.roleLabel : ''}" />
        <c:if test="${empty roleLabel}">
            <c:choose>
                ...
                <c:when test="${role == 'MANAGER'}">
                    <c:set var="roleLabel" value="Ban Quản lý" />
                </c:when>
                <c:when test="${role == 'TENANT'}">
                    <c:set var="roleLabel" value="Người thuê" />
                </c:when>
                <c:when test="${role == 'OPERATOR'}">
                    <c:set var="roleLabel" value="Nhân viên vận hành" />
                </c:when>
            </c:choose>
        </c:if>
        <c:set var="initials"
            value="${not empty sessionScope.currentUser.initials ? sessionScope.currentUser.initials : 'U'}" />
            ...
            <nav class="breadcrumb-nav" aria-label="breadcrumb">
                <ol>
                    ...
                    <c:if test="${empty breadcrumbItems}">
                        <li class="active">
                            <c:out value="${pageHeading != null ? pageHeading : ''}" />
                        </li>
                    </c:if>
                </ol>
            </nav>
                ...
                <div class="user-menu">
                    <c:choose>
                        <c:when test="${not empty sessionScope.currentUser.avatarUrl}">
                            <img src="${ctx}${sessionScope.currentUser.avatarUrl}" class="user-avatar"
