                          ...
                          <div class="widget-surface-body p-0">
                            <table style="width:100%;font-size:0.875rem;border-collapse:collapse">
                              <tr style="border-bottom:1px solid var(--hms-border)">
                                <td style="padding:10px 16px;color:var(--hms-text-muted);width:44%;white-space:nowrap">
                                  Mã phòng</td>
                                ...
                                <td style="padding:10px 16px">
                                  <c:choose>
                                    <c:when test="${not empty room.areaRaw}">
                                      <strong>
                                        <fmt:formatNumber value="${room.area}" maxFractionDigits="1" />
                                      </strong> m²
                                    </c:when>
                                ...
                                <td style="padding:10px 16px">
                                  <c:choose>
                                    <c:when test="${not empty room.roomFee}">
                                      <strong>
                                        <fmt:formatNumber value="${room.roomFee}" pattern="#,###" />
                                      </strong> VNĐ
                                    </c:when>
                                    <c:otherwise><em class="text-muted">Chưa cập nhật</em></c:otherwise>
                                  </c:choose>
                            ...
                            <div class="widget-surface-body">
                              <c:choose>
                                <c:when test="${room.facilityStatus == 'INACTIVE'}">
                                  <div class="alert alert-secondary mb-0" style="font-size:0.8125rem;border-radius:8px">
                                ...
                                <c:otherwise>
                                  <form method="post" action="${ctx}/admin/rooms/${room.id}/update"
                                    ...
                                    <div>
                                      ...
                                      <input type="number" name="area" class="form-control" step="0.1" min="0" max="9999"
                                        placeholder="VD: 25.5" value="${not empty room.areaRaw ? room.area : ''}" />
                                    </div>

                                    <div>
                                      <label class="form-label" style="font-size:0.8125rem">Giá phòng (VNĐ)</label>
                                      <input type="number" name="roomFee" class="form-control" min="0" step="1000"
                                        placeholder="VD: 3000000" value="${not empty room.roomFee ? room.roomFee : ''}" />
                                    </div>
                                    ...
                                    color:var(--hms-ink)">
                                          ...
                                          <table style="font-size:0.8125rem;border-collapse:collapse;width:100%">
                                            <c:if test="${not empty room.tenantCode}">
                                              <tr>
                                                <td style="padding:3px 0;color:var(--hms-text-muted);width:36%">Mã tài
                                                  khoản</td>
                                  ...
                                  <c:otherwise>
                                    <div class="text-center py-4">
                                      <svg width="52" height="52" viewBox="0 0 24 24" fill="none"
                                        stroke="var(--hms-text-muted)" stroke-width="1.2" style="margin-bottom:12px">
                                        <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                                        <circle cx="12" cy="7" r="4" />
                                        <line x1="4" y1="4" x2="20" y2="20" stroke="var(--hms-text-muted)"
                                          stroke-width="1.5" />
                                      </svg>
                                      <div>
                                        <span class="badge-hms badge-neutral"
