              ...
              <div class="card mb-4 border-warning" style="border-radius:var(--hms-radius-md);box-shadow:none;background:#fffdf5">
                <div class="card-header bg-warning bg-opacity-10 font-semibold" style="border-bottom:1px solid #fde68a">
                  <div class="d-flex justify-content-between align-items-center">
                    <span class="fw-bold" style="color:#92400e;font-size:1rem">📋 Chi tiết yêu cầu đã gửi & Tiến độ xử lý</span>
                    <c:choose>
                      <c:when test="${invoice.ticketStatus == 'PENDING'}">
                        <span class="badge bg-warning text-dark px-3 py-2" style="font-size:0.875rem">🟡 Chờ Operator tiếp nhận</span>
                      </c:when>
                ...
                <div class="card-body" style="font-size:0.9rem">
                  <div class="row g-2 mb-3">
                    <div class="col-sm-6">
                      ...
                      <strong><c:out value="${invoice.operatorName}" default="Chưa phân công"/></strong>
                    </div>
                    <div class="col-sm-6">
                      <span class="text-muted d-block" style="font-size:0.8rem">Thời gian gửi:</span>
                      <strong><fmt:formatDate value="${invoice.ticketCreatedAt}" pattern="dd/MM/yyyy HH:mm:ss"/></strong>
                    </div>
                  </div>

                  <div class="mb-3">
                    <span class="text-muted d-block" style="font-size:0.8rem">Tiêu đề yêu cầu:</span>
                    <div class="fw-bold text-dark"><c:out value="${invoice.ticketTitle}" default="${defaultTitle}"/></div>
                  </div>

                  <div class="mb-3">
                    <span class="text-muted d-block" style="font-size:0.8rem">Nội dung chi tiết đã gửi:</span>
                    <div style="background:#f8fafc;padding:10px 14px;border-radius:6px;border:1px solid #e2e8f0;white-space:pre-wrap;font-size:0.875rem"><c:out value="${invoice.ticketContent}" default="${defaultContent}"/></div>
                  </div>

                  <c:if test="${not empty invoice.ticketNotes}">
                    <div class="p-3 bg-light rounded border border-success">
                      <span class="text-success font-semibold d-block" style="font-size:0.85rem">📝 Phản hồi / Kết quả từ Operator:</span>
                      <div class="mt-1" style="white-space:pre-wrap"><c:out value="${invoice.ticketNotes}"/></div>
                    </div>
                  </c:if>
                </div>
              </div>
              ...
              <form method="post" action="${ctx}/manager/notifications/send-operator" class="mt-3">
                ...
                <div class="mb-3">
                  ...
                  <textarea class="form-control" id="content" name="content"
                            rows="6" required maxlength="5000"><c:out value="${defaultContent}"/></textarea>
                ...
                <div class="d-flex gap-2 mt-4">
                  ...
                  <a href="${ctx}/manager/invoices" class="btn-mintlify-secondary text-decoration-none">Quay lại</a>
                </div>
              </form>
            </details>
          </div>
