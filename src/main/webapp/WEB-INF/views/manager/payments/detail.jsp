        ...
        <div>
          <div style="font-size:0.8125rem;color:var(--hms-text-muted);margin-bottom:6px">
            <a href="${ctx}/manager/payments"
               style="color:var(--hms-text-muted);text-decoration:none">← Danh sách giao dịch</a>
          </div>
          <div class="d-flex align-items-center gap-2 flex-wrap">
            <h1 style="margin:0;font-family:monospace"><c:out value="${payment.transactionCode}"/></h1>
            <c:choose>
              <c:when test="${payment.status == 'PENDING'}">
                <span class="badge-hms badge-warning" style="font-size:0.9rem">PENDING</span>
              </c:when>
              <c:when test="${payment.status == 'SUCCESS'}">
                <span class="badge-hms badge-success" style="font-size:0.9rem">SUCCESS</span>
              </c:when>
              <c:otherwise>
                <span class="badge-hms badge-danger" style="font-size:0.9rem">REJECTED</span>
              </c:otherwise>
            </c:choose>
          </div>
          <p style="margin-top:4px">
            Người thuê: <strong><c:out value="${payment.tenantName}"/></strong>
          </p>
        </div>
        <div class="d-flex gap-2 flex-wrap">
          <c:if test="${payment.status == 'PENDING'}">
            <form action="${ctx}/manager/payments/${payment.paymentId}/approve" method="POST" class="m-0">
              ...
              <button type="submit" class="quick-action-btn primary" onclick="return confirm('Bạn có chắc chắn duyệt giao dịch này?');">
            ...
            <form action="${ctx}/manager/payments/${payment.paymentId}/reject" method="POST" class="m-0">
              ...
              <button type="submit" class="quick-action-btn" style="color:var(--hms-accent-danger);border-color:var(--hms-accent-danger);background:transparent" onmouseover="this.style.background='var(--hms-accent-danger)';this.style.color='#fff';" onmouseout="this.style.background='transparent';this.style.color='var(--hms-accent-danger)';" onclick="return confirm('Xác nhận TỪ CHỐI giao dịch này?');">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="margin-right:5px">
            ...
            <div class="widget-surface-body p-0">
              <table style="width:100%;font-size:0.875rem;border-collapse:collapse">
                ...
                <tr style="border-bottom:1px solid var(--hms-border)">
                  ...
                  <td style="padding:10px 16px;text-align:right">
                    <strong><c:out value="${payment.facilityName}"/></strong><br>
                    <small style="color:var(--hms-text-muted)"><c:out value="${payment.facilityAddress}"/></small>
                  </td>
                </tr>
                <tr style="border-bottom:1px solid var(--hms-border)">
                  <td style="padding:10px 16px;color:var(--hms-text-muted)">Phòng</td>
                  <td style="padding:10px 16px;text-align:right">
                    <span class="badge-hms badge-neutral"><c:out value="${payment.roomCode}"/></span>
                  </td>
                </tr>
                <tr style="border-bottom:1px solid var(--hms-border)">
                  <td style="padding:10px 16px;color:var(--hms-text-muted)">Số tiền thanh toán</td>
                  <td style="padding:10px 16px;text-align:right;font-weight:600;color:var(--hms-accent-deep)">
          ...
          <div class="widget-surface h-100">
            ...
            <div class="widget-surface-body p-0">
              <c:choose>
                <c:when test="${not empty payment.invoiceCode}">
                  <table style="width:100%;font-size:0.875rem;border-collapse:collapse">
                    <tr style="border-bottom:1px solid var(--hms-border)">
                      ...
                      <td style="padding:10px 16px;text-align:right;font-weight:500;font-family:monospace;">
                ...
                <c:otherwise>
                  <div class="empty-state p-5 text-center" style="border: 1px dashed var(--hms-border); border-radius:8px; margin: 16px;">
                    <p style="color:var(--hms-text-muted);margin:0">Không có thông tin hóa đơn liên quan</p>
                  </div>
                </c:otherwise>
              </c:choose>
            </div>
