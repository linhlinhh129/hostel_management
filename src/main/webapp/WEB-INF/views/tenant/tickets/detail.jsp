...
<div class="tenant-card"><p><c:out value="${ticket.content}"/></p><small class="text-muted">Gửi lúc: <c:out value="${ticket.createdDateLabel}"/></small><c:if test="${not empty ticket.attachmentUrls1}"><div class="mt-3"><img src="${ctx}${ticket.attachmentUrls1}" alt="�?ính kèm" style="max-width: 100%; border-radius: 8px;"></div></c:if></div>
<c:if test="${not empty ticket.assignedTo}"><div class="tenant-card"><small class="text-muted">Ngư�?i phụ trách: <c:out value="${ticket.assignedTo}"/></small></div></c:if>
