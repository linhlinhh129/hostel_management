                                                ...
                                                <div
                                                    ...
                                                    <div style="flex:2; min-width:160px;">
                                                        ...
                                                        <select name="status" class="form-select" style="width:100%">
                                                            ...
                                                            <option value="PENDING" ${paramStatus=='PENDING'
                                                                ? 'selected' : '' }>Chờ xử lý</option>
                                                            <option value="IN_PROGRESS" ${paramStatus=='IN_PROGRESS'
                                                                ? 'selected' : '' }>Đang xử lý</option>
                                                            <option value="COMPLETED" ${paramStatus=='COMPLETED'
                                                                ? 'selected' : '' }>Đã hoàn thành</option>
                                                            <option value="REJECTED" ${paramStatus=='REJECTED'
                                                                ? 'selected' : '' }>Đã từ chối</option>
                                                        </select>
                                                    </div>
                                                    <div style="flex:2; min-width:160px;">
                                                        <label
                                                            style="display:block; font-size:13px; font-weight:600; color:var(--hms-text-muted); margin-bottom:8px;">Thể
                                                            loại</label>
                                                        ...
                                                        <c:choose>
                                                            ...
                                                            <c:otherwise>
                                                                <c:forEach var="req" items="${requestList}"
                                                                    varStatus="loop">
                                                                    <tr
                                                                        data-href="${ctx}/operator/requests/detail?id=${req.requestId}">
                                                                        <%-- col-code: Geist Mono --%>
                                                                            <td class="d-none d-md-table-cell col-code"
                                                                                style="font-size:13px; color:var(--hms-stone);">
                                                                                <c:out value="${req.code}" />
                                                                            </td>
                                                                            <td
                                                                                style="font-weight:500; color:var(--hms-ink); max-width:200px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
                                                                                <c:out value="${req.title}" />
                                                                            </td>
                                                                            <td class="d-none d-md-table-cell">
                                                                                <c:choose>
                                                                                    <c:when
                                                                                        test="${not empty req.roomCode}">
                                                                                        <span
                                                                                            style="font-weight:600; color:var(--hms-ink);">P.
                                                                                            <c:out
                                                                                                value="${req.roomCode}" />
                                                                                        </span>
                                                                                        <c:if
                                                                                            test="${not empty req.facilityName}">
                                                                                            <span
                                                                                                style="color:var(--hms-stone); font-size:12px;">
                                                                                                (
                                                                                                <c:out
                                                                                                    value="${req.facilityName}" />
                                                                                                )
                                                                                            </span>
                                                                                        </c:if>
                                                                                    </c:when>
                                                                                    <c:otherwise>
                                                                                        <span
                                                                                            style="color:var(--hms-stone); font-size:12px;">—</span>
                                                                                    </c:otherwise>
                                                                                </c:choose>
                                                                            </td>
                                                                            <td class="d-none d-lg-table-cell">
                                                                                <span class="badge-hms badge-neutral" style="white-space:nowrap;">
                                                                                    <c:choose>
                                                                                        <c:when test="${req.category == 'ELECTRIC'}">SỰ CỐ ĐIỆN</c:when>
                                                                                        <c:when test="${req.category == 'WATER'}">SỰ CỐ NƯỚC</c:when>
                                                                                        <c:when test="${req.category == 'UTILITY'}">Điện nước</c:when>
                                                                            ...
                                                                            <td class="d-none d-md-table-cell"
                                                                                ...
                                                                                <fmt:formatDate
                                                                                    value="${req.createdAtAsDate}"
                                                                                    pattern="dd/MM/yyyy HH:mm" />
                                                                            </td>
                                                                            <td>
                                                                                <c:choose>
                                                                                    <c:when test="${req.status == 'PENDING'}"><span class="badge-hms badge-info" style="white-space:nowrap;">Chờ xử lý</span></c:when>
                                                                                    <c:when test="${req.status == 'RECEIVED'}"><span class="badge-hms badge-info" style="white-space:nowrap;">Đã tiếp nhận</span></c:when>
                                                                                    <c:when test="${req.status == 'ASSIGNED'}"><span class="badge-hms badge-neutral" style="white-space:nowrap;">Đã phân công</span></c:when>
                                                                                    <c:when test="${req.status == 'IN_PROGRESS'}"><span class="badge-hms badge-warning" style="white-space:nowrap;">Đang xử lý</span></c:when>
                                                                                    <c:when test="${req.status == 'COMPLETED' or req.status == 'DONE' or req.status == 'RESOLVED'}"><span class="badge-hms badge-success" style="white-space:nowrap;">Hoàn thành</span></c:when>
                                                                                    <c:when test="${req.status == 'REJECTED' or req.status == 'CANCELLED'}"><span class="badge-hms badge-danger" style="white-space:nowrap;">Từ chối</span></c:when>
                                                                                    <c:otherwise><span class="badge-hms badge-neutral" style="white-space:nowrap;"><c:out value="${req.status}"/></span></c:otherwise>
                                                                                </c:choose>
                                                                            </td>
                                                                    </tr>
                                                                </c:forEach>
                                            ...
                                            <%-- table-footer: luôn hiển thị, đồng bộ pattern admin --%>
                                                <div
                                                    ...
                                                    <span class="text-muted" style="font-size:0.875rem">
                                                        Tổng
                                                        <fmt:formatNumber value="${totalRecords}" groupingUsed="true" />
                                                        yêu cầu
