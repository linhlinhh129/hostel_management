...
<style>
  ...
  .page-content button, .page-content label {
    font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
  }
  .req-detail-container { padding: 0.5rem 0 2rem 0; }
  .detail-card {
  ...
  .detail-card-header h3 {
    font-size: 0.9375rem; font-weight: 700; color: var(--hms-ink);
    margin: 0; display: flex; align-items: center; gap: 0.5rem;
  }
  .detail-card-body { padding: 0; flex-grow: 1; }
  .metadata-row {
    display: flex; justify-content: space-between; align-items: flex-start;
    padding: 1rem 1.5rem; border-bottom: 1px solid var(--hms-border-soft);
    transition: background-color 0.15s ease;
  }
  .metadata-row:hover { background-color: var(--hms-surface-soft); }
  .metadata-row:last-child { border-bottom: none; }
  .meta-label {
    display: flex; align-items: center; gap: 0.6rem;
    color: var(--hms-stone); font-size: 0.875rem; font-weight: 500; flex-shrink: 0;
  }
  .meta-label svg { color: var(--hms-muted); }
  .meta-value {
    color: var(--hms-ink); font-size: 0.875rem; font-weight: 600;
    text-align: right; max-width: 60%; word-break: break-word;
  }
  .req-btn-outline {
    background: var(--hms-canvas); color: var(--hms-text-secondary);
    font-family: 'Inter', sans-serif; font-size: 0.875rem; font-weight: 600;
    padding: 0.5rem 1.25rem; height: 40px; border-radius: var(--hms-radius);
    border: 1px solid var(--hms-border); display: inline-flex; align-items: center;
    justify-content: center; gap: 0.4rem; cursor: pointer; text-decoration: none;
    transition: background 0.15s, border-color 0.15s;
  }
  .req-btn-outline:hover { background: var(--hms-surface); border-color: var(--hms-stone); }
  .req-btn-primary {
    font-family: 'Inter', sans-serif; font-size: 0.875rem; font-weight: 600;
    padding: 0.5rem 1.25rem; height: 40px; border-radius: var(--hms-radius);
    border: none; display: inline-flex; align-items: center; justify-content: center;
    gap: 0.4rem; cursor: pointer; transition: opacity 0.15s, transform 0.15s; width: 100%;
  }
  .req-btn-primary:hover { opacity: 0.88; transform: translateY(-1px); }
  .req-btn-green  { background: var(--hms-success); color: #fff; }
  .req-btn-sky    { background: #0ea5e9; color: #fff; }
  .req-btn-red    { background: var(--hms-danger); color: #fff; }
</style>
  ...
  <div class="main-wrapper">
    ...
    <main class="page-content">
      ...
      <c:if test="${not empty error}">
        <div class="alert alert-danger border-0" style="border-radius:var(--hms-radius);background:var(--hms-danger-bg);color:var(--hms-danger);margin-bottom:16px;">
          <c:out value="${error}"/>
        </div>
      </c:if>
      <c:if test="${not empty sessionScope.successMessage}">
        <div class="alert alert-success border-0" style="border-radius:var(--hms-radius);background:var(--hms-success-bg);color:var(--hms-success);margin-bottom:16px;">
      ...
      <div class="req-detail-container">
        ...
        <div class="page-header hero-sky-gradient d-flex justify-content-between align-items-center"
          ...
          <div>
            ...
            <h1><c:out value="${reqDetail.title}"/></h1>
            <p style="margin:0;">
              Mã yêu cầu: <span style="font-family:'Geist Mono','JetBrains Mono',monospace;font-weight:600;">
        ...
        <div class="row g-4">
          ...
          <div class="col-lg-7">
            <div class="detail-card">
              <div class="detail-card-header">
                <h3>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--hms-info)"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/></svg>
                  Nội dung yêu cầu
                </h3>
              </div>
          ...
          <div class="col-lg-5">
            <div class="detail-card">
              <div class="detail-card-header">
                <h3>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" style="color:var(--hms-success)"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                  Thông tin chi tiết
                </h3>
              </div>
              <div class="detail-card-body">
                ...
                <div class="metadata-row">
                  ...
                  <span class="meta-value"><c:out value="${reqDetail.senderName}"/></span>
                </div>
                ...
                <div class="metadata-row">
                  ...
                  <span class="meta-value">
                    P.<c:out value="${reqDetail.roomCode}"/> — <c:out value="${reqDetail.facilityName}"/>
                  </span>
                </div>
              ...
              <%-- ── Action footer ──────────────────────────────────── --%>
              <div style="padding:1.25rem 1.5rem;border-top:1px solid var(--hms-border-soft);background:var(--hms-surface-soft);">
                <div style="font-size:0.8125rem;font-weight:600;color:var(--hms-stone);text-transform:uppercase;letter-spacing:0.4px;margin-bottom:0.75rem;">
                  Hành động
                </div>
                ...
                <c:choose>
                  ...
                  <c:when test="${reqDetail.status == 'PENDING'}">
                    <form action="${ctx}/operator/requests/detail" method="POST" class="m-0 mb-2">
                    ...
                    <button type="button" onclick="openRejectModal()"
                            class="req-btn-primary" style="background:var(--hms-canvas);color:var(--hms-text-secondary);border:1px solid var(--hms-border);">
                  ...
                  <c:when test="${reqDetail.status == 'ASSIGNED' && reqDetail.assignedStaffId == sessionScope.currentUser.id}">
                    <c:if test="${not empty reqDetail.appointSchedule}">
                      <div class="badge-hms badge-info" style="display:block;margin-bottom:10px;font-size:12px;font-weight:400;border-radius:var(--hms-radius-sm);padding:8px 10px;">
                    ...
                    <form action="${ctx}/operator/requests/detail" method="POST" class="m-0"
                      ...
                      <input type="datetime-local" id="appointmentDateInput" name="appointmentDate"
                             ...
                             class="form-control mb-2" style="border-radius:var(--hms-radius);font-size:14px;" required/>
                  ...
                  <c:when test="${reqDetail.status == 'IN_PROGRESS' && reqDetail.assignedStaffId == sessionScope.currentUser.id}">
                    <button type="button" onclick="openCompleteModal()" class="req-btn-primary req-btn-green">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                      Báo cáo hoàn thành
                    </button>
                  </c:when>
                  ...
                  <c:otherwise>
                    <div style="text-align:center;padding:12px;background:var(--hms-surface);
                      ...
                      <c:choose>
                        <c:when test="${reqDetail.status == 'IN_PROGRESS'}">Yêu cầu đang được xử lý bởi nhân sự khác.</c:when>
                        <c:when test="${reqDetail.status == 'COMPLETED' || reqDetail.status == 'DONE'}">Yêu cầu đã được xử lý thành công.</c:when>
                        <c:when test="${reqDetail.status == 'REJECTED'}">Yêu cầu đã bị từ chối.</c:when>
  ...
  <div class="custom-modal-dialog">
    ...
    <form action="${ctx}/operator/requests/detail" method="POST">
      ...
      <div class="d-flex justify-content-end mt-4" style="gap:12px;">
        ...
        <button type="submit" class="req-btn-primary req-btn-red" style="width:auto;">Xác nhận từ chối</button>
      </div>
    </form>
  </div>
</div>
...
<div id="completeModal" class="custom-modal-backdrop" style="z-index:9999;">
  <div class="custom-modal-dialog">
    ...
    <form action="${ctx}/operator/requests/detail" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        <label class="d-flex align-items-center gap-2 mb-2" style="font-size:14px;font-weight:500;color:#111827;cursor:pointer;">
      ...
      <div class="mb-3" id="image_upload_section">
        <label class="d-block mb-2" style="font-size:14px;font-weight:500;color:#111827;">
          Ảnh minh chứng (tối đa 5 ảnh) <span style="color:var(--hms-danger)" id="image_required_star">*</span>
        </label>
        ...
        <div style="font-size:12px;color:#6b7280;margin-top:4px;">Định dạng: JPG, PNG. Tối đa 5MB/ảnh.</div>
      </div>
      <div class="d-flex justify-content-end mt-4" style="gap:12px;">
        <button type="button" onclick="closeCompleteModal()" class="req-btn-outline">Hủy</button>
        <button type="submit" class="req-btn-primary req-btn-green" style="width:auto;">Xác nhận lưu</button>
      </div>
    </form>
  </div>
</div>
