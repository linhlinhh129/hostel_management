...
UNION ALL SELECT 'community_posts', COUNT(*) FROM dbo.community_posts
UNION ALL SELECT 'post_reactions', COUNT(*) FROM dbo.post_reactions
...
BEGIN TRANSACTION;
BEGIN TRY
    PRINT N'=== BẮT ĐẦU XÓA TẤT CẢ DỮ LIỆU CỦA NGUYỄN TIẾN QUỐC ===';

    -- 0. Tạo bảng chứa danh sách User ID cần xóa
    DECLARE @user_ids TABLE (user_id INT);
    INSERT INTO @user_ids (user_id)
    SELECT user_id FROM dbo.users 
    WHERE full_name LIKE N'%Nguyễn Tiến Quốc%' OR username LIKE N'%quoc%';

    -- 1. Giải phóng Phòng trọ về trạng thái TRỐNG (AVAILABLE)
    UPDATE dbo.rooms
    SET tenant_id = NULL,
        status = 'AVAILABLE',
        contract_start_date = NULL,
        contract_end_date = NULL
    WHERE tenant_id IN (SELECT user_id FROM @user_ids)
       OR room_id IN (SELECT room_id FROM dbo.contracts WHERE tenant_full_name LIKE N'%Nguyễn Tiến Quốc%');

    -- 2. Xóa Người phụ thuộc (dbo.dependents)
    IF OBJECT_ID('dbo.dependents', 'U') IS NOT NULL
        DELETE FROM dbo.dependents WHERE tenant_id IN (SELECT user_id FROM @user_ids);

    -- 3. Xóa Phản ứng bài đăng (dbo.post_reactions)
    IF OBJECT_ID('dbo.post_reactions', 'U') IS NOT NULL
        DELETE FROM dbo.post_reactions WHERE user_id IN (SELECT user_id FROM @user_ids);

    -- 4. Xóa Bình luận bài đăng (dbo.post_comments)
    IF OBJECT_ID('dbo.post_comments', 'U') IS NOT NULL
        DELETE FROM dbo.post_comments WHERE user_id IN (SELECT user_id FROM @user_ids);

    -- 5. Xóa Bài đăng cộng đồng (dbo.community_posts)
    IF OBJECT_ID('dbo.community_posts', 'U') IS NOT NULL
        DELETE FROM dbo.community_posts WHERE author_id IN (SELECT user_id FROM @user_ids) OR reviewed_by IN (SELECT user_id FROM @user_ids);

    -- 6. Xóa Người nhận thông báo & Thông báo (dbo.notification_recipients & dbo.notifications)
    IF OBJECT_ID('dbo.notification_recipients', 'U') IS NOT NULL
    BEGIN
        EXEC sp_executesql N'DELETE FROM dbo.notification_recipients WHERE user_id IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%'')';
        EXEC sp_executesql N'DELETE FROM dbo.notification_recipients WHERE notification_id IN (SELECT notification_id FROM dbo.notifications WHERE created_by IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%''))';
    END

    IF OBJECT_ID('dbo.notifications', 'U') IS NOT NULL
        DELETE FROM dbo.notifications WHERE created_by IN (SELECT user_id FROM @user_ids);

    -- 7. Xóa Thanh toán (dbo.payments) - Dùng Dynamic SQL để tránh lỗi compile-time
    IF OBJECT_ID('dbo.payments', 'U') IS NOT NULL
    BEGIN
        IF COL_LENGTH('dbo.payments', 'created_by') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.payments WHERE created_by IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%'')';

        IF COL_LENGTH('dbo.payments', 'tenant_id') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.payments WHERE tenant_id IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%'')';

        IF COL_LENGTH('dbo.payments', 'contract_id') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.payments WHERE contract_id IN (SELECT contract_id FROM dbo.contracts WHERE tenant_full_name LIKE N''%Nguyễn Tiến Quốc%'')';
    END

    -- 8. Xóa Hóa đơn (dbo.invoices) - Dùng Dynamic SQL để tránh lỗi compile-time
    IF OBJECT_ID('dbo.invoices', 'U') IS NOT NULL
    BEGIN
        IF COL_LENGTH('dbo.invoices', 'created_by') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.invoices WHERE created_by IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%'')';

        IF COL_LENGTH('dbo.invoices', 'tenant_id') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.invoices WHERE tenant_id IN (SELECT user_id FROM dbo.users WHERE full_name LIKE N''%Nguyễn Tiến Quốc%'' OR username LIKE N''%quoc%'')';

        IF COL_LENGTH('dbo.invoices', 'contract_id') IS NOT NULL
            EXEC sp_executesql N'DELETE FROM dbo.invoices WHERE contract_id IN (SELECT contract_id FROM dbo.contracts WHERE tenant_full_name LIKE N''%Nguyễn Tiến Quốc%'')';
    END

    -- 9. Xóa Yêu cầu hỗ trợ (dbo.requests)
    IF OBJECT_ID('dbo.requests', 'U') IS NOT NULL
        DELETE FROM dbo.requests WHERE sender_id IN (SELECT user_id FROM @user_ids) OR assigned_staff_id IN (SELECT user_id FROM @user_ids);

    -- 10. Xóa Nhật ký hệ thống (dbo.audit_logs)
    IF OBJECT_ID('dbo.audit_logs', 'U') IS NOT NULL
        DELETE FROM dbo.audit_logs WHERE created_by IN (SELECT user_id FROM @user_ids);

    -- 11. Cập nhật Chốt số điện nước (dbo.meter_readings)
    IF OBJECT_ID('dbo.meter_readings', 'U') IS NOT NULL
        UPDATE dbo.meter_readings SET created_by = NULL WHERE created_by IN (SELECT user_id FROM @user_ids);

    -- 12. Xóa Hợp đồng (dbo.contracts)
    DELETE FROM dbo.contracts 
    WHERE tenant_full_name LIKE N'%Nguyễn Tiến Quốc%' 
       OR tenant_id IN (SELECT user_id FROM @user_ids)
       OR created_by IN (SELECT user_id FROM @user_ids);

    -- 13. Cuối cùng, Xóa Tài khoản Người dùng trong dbo.users
    DELETE FROM dbo.users 
    WHERE full_name LIKE N'%Nguyễn Tiến Quốc%' 
       OR username LIKE N'%quoc%';

    COMMIT TRANSACTION;
    PRINT N'>>> THÀNH CÔNG RỰC RỠ: Đã xóa hoàn toàn Nguyễn Tiến Quốc và giải phóng phòng trọ! <<<';
END TRY
BEGIN CATCH
    IF @@TRANCOUNT > 0
        ROLLBACK TRANSACTION;
