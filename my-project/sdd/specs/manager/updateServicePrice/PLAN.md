...
- Cho phép Ban quản lý cập nhật giá mới.
...
- Hiển thị lịch sử cập nhật (old_price, new_price, người cập nhật, thời gian).
- Áp dụng Validation từ phía Server để đảm bảo tính toàn vẹn (định dạng số nguyên không chứa ký tự phân cách, > 0, khác giá cũ).
...
- **Cập nhật giá mới (Update):** Form gửi POST về `/manager/service-prices?action=update`. Server parse số tiền, validate định dạng nguyên dương không dấu phẩy/chấm và khác giá cũ, cập nhật bảng `facilities` trong 1 Transaction, đồng thời insert 1 record vào bảng lịch sử. Chuyển hướng lại trang danh sách nếu thành công. Nếu thất bại, forward lại `index.jsp` kèm `errorMessage`.
...
- **POST `/manager/service-prices?action=update`**: Form submit với các params `priceType`, `newPrice`. 

## Quy tắc nghiệp vụ
- `newPrice` phải là số nguyên dương không chứa dấu phân cách, khác giá cũ và > 0. Nếu không, bắt Exception và forward lại form với lỗi.
