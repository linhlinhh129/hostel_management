...
- **Layout tổng thể:** Tái sử dụng Sidebar và Navbar hiện có. Vùng nội dung chính (Feed) được căn giữa, chiều rộng tối ưu 700-800px trên Desktop và responsive 100% trên Mobile/Tablet. Nền trang màu xám nhạt (`#f5f6fa`) để làm nổi bật các bài viết. Bên phải có thêm Widget "Top Bài Nổi Bật".
...
- **Hình ảnh:** Tự động căn chỉnh full-width bên trong thẻ, bo góc 12px, cắt cúp `object-fit: cover`, giới hạn chiều cao 350-450px để khung hình luôn cân đối. Khi click vào ảnh, hiển thị popup (modal) phóng to hình ảnh để xem chi tiết.
...
- **Tương tác (Fetch API):** Cơ chế Thích, Bình luận và Xóa bình luận hoạt động tức thời (Real-time feel) mà không cần tải lại trang. Các bình luận cũ nhất xếp trên, mới nhất xếp dưới. Xóa bình luận sẽ có popup yêu cầu xác nhận.
- **Widget "Top Bài Nổi Bật":** Hiển thị ở cột bên phải, gọi API lấy top 5 bài viết có tương tác cao nhất. Click vào sẽ chuyển sang trang chi tiết bài viết.
...
- **Servlet & API:** 
  ...
  - Các API nội bộ cung cấp dữ liệu JSON:
    - `GET /api/v1/news-feed`: Lấy danh sách bài viết (Load More).
    - `GET /api/v1/news-feed/top`: Lấy top 5 bài viết nổi bật.
    - `POST /api/v1/posts/{postId}/reactions`: Toggle Thích / Bỏ thích.
...
- **DAO & Tối ưu:** 
  ...
  - Cần thêm truy vấn để lấy danh sách top 5 bài viết có tổng lượt tương tác (Like + Comment) cao nhất.
...
1. Bộ icon sử dụng sẽ ưu tiên dùng thư viện có sẵn (FontAwesome / Bootstrap Icons) đang được tích hợp trong dự án. Bạn có đồng ý không?
2. Nút "Tải thêm bài viết" sẽ được đặt ở cuối danh sách feed theo đúng yêu cầu trong Spec. Cần xác định mỗi lần bấm sẽ tải thêm bao nhiêu bài (VD: limit = 10)?
