...
* **Thuật toán băm mật khẩu:** Bắt buộc sử dụng `Argon2id` (memory: 64MB, time: 3, threads: 4).
...
1. **LoginServlet** (`controller.auth.LoginServlet`):
   ...
   - Quản lý điều hướng:
     - Đăng nhập lần đầu (cờ `force_change_pass = true`): Lưu vào session kèm cờ `firstLogin = true` -> Redirect HTTP 302 về `/auth/force-change-password`.
...
2. **UserService** / **UserServiceImpl**:
   ...
   - Sử dụng `PasswordUtil` để kiểm tra độ khớp mật khẩu (Argon2id).
...
5. **PasswordUtil**:
   - Sử dụng thư viện `argon2-jvm` cung cấp hàm `verify` mã băm Argon2id.
6. **AuthFilter**:
...
* **Response Thành công:** Redirect 302 tới Dashboard hoặc trang đổi mật khẩu.
