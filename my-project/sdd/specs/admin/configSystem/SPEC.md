...
- **WHEN** Quản trị viên để trống bất kỳ trường bắt buộc nào (`host`, `port`, `username`)  
  **THE SYSTEM SHALL** hiển thị thông báo lỗi và không lưu cấu hình. (Lưu ý: `password` có thể để trống để giữ nguyên mật khẩu cũ).
...
| **Phân quyền** | Role = `ADMIN` (kiểm tra qua `RoleFilter`) | Role = `ADMIN` (`RoleFilter`) | Role = `ADMIN` (`RoleFilter`) |
...
| `username` | `String` | ✔ | Tài khoản Email dùng để gửi (cũng là địa chỉ gửi — From:) |
| `password` | `String` | ✘ (Optional) | Mật khẩu ứng dụng (App Password). Nếu để trống sẽ giữ nguyên mật khẩu cũ. |
...
|---|---|---|---|
| `emailConfig` | `EmailConfigDTO` | `SystemConfigService.getUIEmailConfig()` | Cấu hình Email hiện tại (password không có trong DTO — luôn mask ở UI) |
| `vnpayConfig` | `VNPayConfigDTO` | `SystemConfigService.getUIVNPayConfig()` | Cấu hình VNPay hiện tại (secretKey không có trong DTO — luôn mask ở UI) |
...
| Tình huống | Hành vi |
|---|---|
| Chưa đăng nhập | Redirect về `/login` (kiểm tra ở đầu `doGet`/`doPost` của Servlet) |
