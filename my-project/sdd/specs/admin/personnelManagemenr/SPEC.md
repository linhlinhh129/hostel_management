...
As an Admin, I want to quản lý danh sách nhân sự để có thể kiểm soát tài khoản và quyền truy cập của từng nhân sự trong hệ thống.

### Story 2 (Happy Path)

As an Admin, I want to xem thông tin chi tiết nhân sự để có thể theo dõi đầy đủ thông tin tài khoản và phạm vi cơ sở được phân công.

### Story 3 (Happy Path)

As an Admin, I want to tạo mới nhân sự và gán vai trò để nhân sự có thể đăng nhập và sử dụng các chức năng phù hợp với quyền hạn được cấp.

### Story 4 (Happy Path)

As an Admin, I want to cập nhật thông tin nhân sự để đảm bảo dữ liệu luôn chính xác.

### Story 5 (Edge Case)

As an Admin, when email hoặc số điện thoại đã tồn tại trong hệ thống, I want to nhận được thông báo lỗi phù hợp.

### Story 6 (Edge Case)

As an Admin, when một nhân sự nghỉ việc hoặc tạm ngưng công tác, I want to khóa tài khoản để ngăn truy cập vào hệ thống.

### Story 7 (Edge Case)

As an Admin, when nhân sự có vai trò Ban Quản Lý hoặc Nhân viên vận hành chưa được gán cơ sở quản lý, I want hệ thống từ chối tạo mới hoặc cập nhật để đảm bảo nhân sự luôn có phạm vi quản lý rõ ràng.
...
```http
GET /api/v1/employees?page=1&size=20&keyword=minh
```
...
```json
{
  ...
  "error": {
    ...
    "message": "Nhân sự phải được gán ít nhất một cơ sở quản lý"
  }
}
```
...
Hỗ trợ phân trang tối thiểu 20 bản ghi/trang.
