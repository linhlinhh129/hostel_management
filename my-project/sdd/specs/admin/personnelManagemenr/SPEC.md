...
| Thuộc tính | Giá trị |
|---|---|
| **Servlet** | `AdminPersonnelServlet` |
| **URL Pattern** | `GET /admin/personnel` — danh sách |
| **URL Pattern** | `GET /admin/personnel/create` — form tạo |
| **URL Pattern** | `POST /admin/personnel/create` — lưu tạo mới |
| **URL Pattern** | `GET /admin/personnel/{id}` — chi tiết |
...
### 4.2 Query Parameters (GET /admin/personnel)

| Tham số | Kiểu | Mô tả |
|---|---|---|
| `keyword` | `String` | Tìm theo fullName, username, email, phone |
| `role` | `String` | Filter theo `MANAGER` hoặc `OPERATOR` |
| `status` | `String` | Filter theo `ACTIVE` hoặc `INACTIVE` |
...
|---|---|---|---|
| `page` | `PageDTO<User>` | `PersonnelDAO.findAll(keyword, role, status, page, 20)` | Dữ liệu phân trang, `PAGE_SIZE = 20` |
| `keyword` | `String` | Query param `keyword` | Giữ lại giá trị tìm kiếm |
| `selectedRole` | `String` | Query param `role` | Giữ lại filter role |
| `selectedStatus` | `String` | Query param `status` | Giữ lại filter status |

Mỗi `User` trong danh sách được enrich thêm `facilityNames` qua `PersonnelDAO.findFacilityNamesForUser(id)`.
...
| Attribute | Java Type | Nguồn dữ liệu | Mô tả |
|---|---|---|---|
| `user` | `User` | `PersonnelDAO.findById(id)` | Thông tin nhân sự, kèm `facilityNames` |

---

### 4.5 Request Attributes — Form tạo (create.jsp)

| Attribute | Java Type | Nguồn dữ liệu | Mô tả |
|---|---|---|---|
| `managerFacilities` | `List<Facility>` | `PersonnelDAO.findFacilitiesForManager(null)` | Cơ sở ACTIVE chưa có MANAGER |
| `operatorFacilities` | `List<Facility>` | `PersonnelDAO.findFacilitiesForOperator(null)` | Cơ sở ACTIVE chưa có OPERATOR |
| `errorMessage` | `String` | `ValidationException.getMessage()` | Lỗi khi submit thất bại |
| `dto` | `Map<String, String>` | Build từ request params | Giữ lại dữ liệu đã nhập khi form lỗi |

---

### 4.6 Request Attributes — Form sửa (edit.jsp)

| Attribute | Java Type | Nguồn dữ liệu | Mô tả |
|---|---|---|---|
| `user` | `User` | `PersonnelDAO.findById(id)` | Thông tin nhân sự hiện tại |
| `currentFacilityId` | `Integer` | `PersonnelDAO.findFacilityIdForUser(id)` | ID cơ sở đang được gán |
| `managerFacilities` | `List<Facility>` | `PersonnelDAO.findFacilitiesForManager(id nếu MANAGER)` | Kèm cơ sở hiện tại nếu đang là MANAGER |
| `operatorFacilities` | `List<Facility>` | `PersonnelDAO.findFacilitiesForOperator(id nếu OPERATOR)` | Kèm cơ sở hiện tại nếu đang là OPERATOR |
| `errorMessage` | `String` | `ValidationException.getMessage()` | Lỗi khi submit thất bại |

---

### 4.7 Validation — POST create / edit

| Form param | Điều kiện hợp lệ | Lỗi ném ra |
|---|---|---|
| `fullName` | Không rỗng | `ValidationException` |
| `email` | Không rỗng, đúng format, chưa tồn tại | `ValidationException` |
| `phone` | Số điện thoại Việt Nam 10 số | `ValidationException` |
| `role` | `MANAGER` hoặc `OPERATOR`, không phải `ADMIN` | `ValidationException` |
| `identityNumber` | 9 hoặc 12 chữ số, chưa tồn tại | `ValidationException` |
| `dob` | Không được lớn hơn ngày hiện tại | `ValidationException` |
| `facilityId` | Bắt buộc nếu role là MANAGER/OPERATOR | `ValidationException` |
| `facilityId` | Cơ sở phải tồn tại và status = `ACTIVE` | `ValidationException` |
| MANAGER conflict | Cơ sở chưa có MANAGER khác | `ValidationException` |
| OPERATOR conflict | Cơ sở chưa có OPERATOR khác | `ValidationException` |

---

### 4.8 Hành vi đặc biệt

**Tạo mới:**
- Username = email
- Mật khẩu tạm được sinh ngẫu nhiên qua `PasswordUtil.generateTempPassword()`, hash bcrypt
- Gửi email qua `EmailService.sendTempPassword()` (async, không block nếu lỗi)
